	...
	var GhostPanel = Widget.extend({
		...
		_initPanel: function() {
			...
			if(this.element.parent().length === 0) {
				this.element.appendTo($('body')).css('visibility', 'hidden');
			}
		...
		_scrollScreen: function() {
			...
			if(this.isIE6) {
				...
				this.threadId = setTimeout(function() {
					// 新位置的 CSS
					var css = {
						'left': $(document).scrollLeft() + _self._getHorizontalPos(_self.element) + 'px',
						'top': $(document).scrollTop() + _self._getVerticalPos(_self.element) + 'px',
						'visibility': 'visible'
					};

					// 如果本来是隐藏的或使用淡入淡出效果, 淡入显示
					if(effect === FADE || _self.element.is(':hidden') || _self.element.css('visibility') === 'hidden') {
						css.opacity = 0;
						_self.element.css(css).animate({'opacity': 1});
					} else {
						_self.element.animate(css, 800);
					}
				}, 400);
			...
			// 在隐藏区域之外, 其他浏览器显示节点
				...
				var css = {
					'left': this._getHorizontalPos(this.element) + 'px',
					'top': this._getVerticalPos(this.element) + 'px',
					'visibility': 'visible'
				};

				if(this.element.css('visibility') === 'hidden') {
					css.opacity = 0;
					this.element.css(css).animate({'opacity': 1});
				} else {
		...
		_resizeWindow: function() {
			var left = this._getHorizontalPos(this.element);
			var top = this._getVerticalPos(this.element);
